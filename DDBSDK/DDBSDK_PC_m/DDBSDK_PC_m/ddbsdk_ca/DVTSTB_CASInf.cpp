...
{
#if 1
	BYTE Devic_UUID[16] = {0x85, 0xc3, 0x1d, 0x45, 0x5f, 0xbc, 0x05, 0x5a, 0xe7, 0x56, 0xd9, 0x55, 0xc9, 0x28, 0xb0, 0x78};
	memcpy(pbyUUid, Devic_UUID, 16);
#else
...
HRESULT DVTSTBCA_WriteDataBuffer(const char * pStartAddr,long * plDataLen,const unsigned char* pData)
{
	...
	try
	{
		...
		if((cafile = fopen("softcard.ca", "wb")) == NULL)
		{
			return -1;
		}
