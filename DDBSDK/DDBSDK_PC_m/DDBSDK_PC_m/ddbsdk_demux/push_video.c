...
uint32_t pushPacketVideoPES2(SLConfig_DR * sl_config_dr, uint8_t * in_Buf, int * headerLength, unsigned long * pkgLenght)
{
	...
	uint8_t *  temp_ptr = in_Buf;
	uint8_t *  temp_ptr_start = NULL;
...
// 	uint8_t ** pes_buf = &param1->dvb_video_ptr;
	...
	if((in_Buf[3] & 0x30) == 0x30) {

		adap_field_length = in_Buf[4];
		temp_ptr += adap_field_length + 1;
	} else if((in_Buf[3] & 0x30) == 0x20){
		...
		default:
			if((stream_id >= 0xF4 && stream_id <= 0xFE) || stream_id < 0xBC) {
				...
				if(stream_id == SEC14496_SL_PACKED_STREAM) {
					...
					push_14496SL_Pack(sl_config_dr, &sl_pack, temp_ptr);
					temp_ptr = sl_pack.payload;
					
					*pkgLenght = (temp_ptr[0] << 24) | (temp_ptr[1] << 16) | (temp_ptr[2] << 8) | temp_ptr[3];
					temp_ptr = &temp_ptr[4];
...
// 					temp_ptr += 4;
// 					pack_copy_len = TS_VALID_COUNT - (temp_ptr - in_Buf);
// 					memcpy(out_Buf, temp_ptr, pack_copy_len);
#if 0
...
(* pes_buf) += 4;
				...
				*pkgLenght = pes_pack.pes_packet_length - (temp_ptr - temp_ptr_start);
...
// 				memcpy(out_Buf, temp_ptr, pack_copy_len);
		...
		//memcpy(out_Buf, temp_ptr, lenght);
	}

	*headerLength = temp_ptr - in_Buf;
