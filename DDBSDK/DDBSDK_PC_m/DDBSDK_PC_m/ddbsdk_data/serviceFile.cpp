...
// 	WriteLog("saveServiceFileData::g_servTab:%x", g_servTab);
	...
	WriteLog("saveServiceFileData::isRunning:%x", isRunning);
	WriteLog("saveServiceFileData::isRunning是否为1:%d", *isRunning);

	HANDLE downloadFileHandle = NULL;
	memcpy(&downloadFileHandle, &pOutBuf[sizeof(BOOL *)], sizeof(HANDLE));
	WriteLog("saveServiceFileData::downloadFileHandle:%x", downloadFileHandle);

	u16 pid = 0;
	memcpy(&pid, &pOutBuf[sizeof(BOOL *) + sizeof(HANDLE)], sizeof(u16));
	WriteLog("saveServiceFileData::PID:%x", pid);
	...
	for(num=0; num<9; num++) {
		...
		if(fopen(filePath_STATUS, "wb") == NULL) {
...
// 	WriteLog("initServiceFileData::servTab_p:%x", servTab_p);
	...
	memcpy(&(this->g_ddbsdk_servFileBuffer[sizeof(BOOL *)/*sturctSize*/]), &(this->downHandle), sizeof(HANDLE));
	WriteLog("initServiceFileData::(this->downHandle):%x",(this->downHandle));

	u16 pid = this->prog_pid;
	memcpy(&(this->g_ddbsdk_servFileBuffer[sizeof(BOOL *) + sizeof(HANDLE)]), &pid, sizeof(u16));
	WriteLog("initServiceFileData::(pid:%x",(pid));
	//::g_servTab_p = &(this->g_servTab);
	//::downloadFileHandle = this->downHandle;
	/* create a thread process file data */
	this->hCheckServiceFile = (HANDLE)_beginthreadex(NULL, 0, saveServiceFileData, this->g_ddbsdk_servFileBuffer, 0, &dwThreadID);
