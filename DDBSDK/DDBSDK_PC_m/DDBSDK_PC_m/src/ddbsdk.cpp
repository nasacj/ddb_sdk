...
int checkAuthorized()
{
	...
	HANDLE localFile = CreateFile(L"ddbsdk.lic", GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
	int error = GetLastError();
	if (INVALID_HANDLE_VALUE == localFile)
	{
		copyFile("ddbsdk.lic.back", "ddbsdk.lic");
		localFile = CreateFile(L"ddbsdk.lic", GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
	}
...
// 	if (checkSignal_debug)
// 	{
// 		hCheckSignal = _beginthreadex(NULL, 0, thread_checkSignal, &nParameter, 0, &dwThreadID);
// 	}
...
DWORD WINAPIV DDB_Read(DWORD Handle, LPVOID pBuffer, DWORD dwNumBytes)
{
	printf("This is DAB_Read\n");

	return DWORD(1);;
}
DWORD WINAPIV DDB_Write(DWORD Handle, LPCVOID pBuffer, DWORD dwNumBytes)
{
	printf("This is DAB_Write\n");
	return DWORD(1);;
}
DWORD WINAPIV DDB_Seek(DWORD Handle, long lDistance, DWORD dwMoveMethod)
{
	printf("This is DAB_Seek\n");
	return DWORD(1);;
}

void WINAPIV DDB_PowerUp(void)
{
