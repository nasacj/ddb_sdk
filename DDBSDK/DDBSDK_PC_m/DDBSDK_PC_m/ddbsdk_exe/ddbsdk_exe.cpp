...
void ddbif_dataListTest(void)
{
	...
	if(deviceOpen == false) {
		...
		//} else {
		//	hThread_1 = _beginthreadex(NULL, 0, thread_sendStreamData, &nParameter_1, 0, &dwThreadID_1);
		//}
		deviceOpen = true;
	}
...
unsigned int __stdcall thread_getStreamVideo(void * param)
{
...
unsigned int __stdcall thread_getStreamAudio(void * param)
{
...
{
	unsigned int dwThreadID_1, dwThreadID_2, dwThreadID_3;
...
// 			DDB_IOControl(NULL, IOCTL_DDBSDK_OPEN_DMBDEVICE, NULL, 0, NULL, 0, NULL);
// 		} else {
// 			hThread_1 = _beginthreadex(NULL, 0, thread_sendStreamVideo, &nParameter_1, 0, &dwThreadID_1);
// 		}
// 		deviceOpen = true;
// 	}
	...
	hThread_2 = (HANDLE)_beginthreadex(NULL, 0, thread_getStreamVideo, &nParameter_2, 0, &dwThreadID_2);

	/* create thread to get audio */
	hThread_3 = (HANDLE)_beginthreadex(NULL, 0, thread_getStreamAudio, &nParameter_3, 0, &dwThreadID_3);
...
unsigned int __stdcall _thread_show_typeFile(void* param)
{
...
unsigned int __stdcall Thread_test_downloadFile(void* param)
{
	BOOL isStartTick = FALSE;
	DWORD testCount = 0;
	DWORD startTime = 0;
	{
		...
		unsigned int filetype_5_H_ID = 0;
		unsigned int filetype_8_H_ID = 0;
		//unsigned int filetype_10_H_ID = 0;
		unsigned int filetype_11_H_ID = 0;
		unsigned int filetype_12_H_ID = 0;
		unsigned int filetype_15_H_ID = 0;
		unsigned int filetype_19_H_ID = 0;
		
		HANDLE filetype_5_H = (HANDLE)_beginthreadex(NULL, 0, _thread_show_typeFile, &type5, 0, &filetype_5_H_ID);
		HANDLE filetype_8_H = (HANDLE)_beginthreadex(NULL, 0, _thread_show_typeFile, &type8, 0, &filetype_8_H_ID);
		//HANDLE filetype_10_H = (HANDLE)_beginthreadex(NULL, 0, _thread_show_typeFile, &type10, 0, &filetype_10_H_ID);
		HANDLE filetype_11_H = (HANDLE)_beginthreadex(NULL, 0, _thread_show_typeFile, &type11, 0, &filetype_11_H_ID);
		HANDLE filetype_12_H = (HANDLE)_beginthreadex(NULL, 0, _thread_show_typeFile, &type12, 0, &filetype_12_H_ID);
		HANDLE filetype_15_H = (HANDLE)_beginthreadex(NULL, 0, _thread_show_typeFile, &type15, 0, &filetype_15_H_ID);
		HANDLE filetype_19_H = (HANDLE)_beginthreadex(NULL, 0, _thread_show_typeFile, &type19, 0, &filetype_19_H_ID);
		...
		CloseHandle(filetype_5_H);
		CloseHandle(filetype_8_H);
		//CloseHandle(filetype_10_H);
		CloseHandle(filetype_11_H);
		CloseHandle(filetype_12_H);
		CloseHandle(filetype_15_H);
		CloseHandle(filetype_19_H);
// 		ddbsdk_servFileStatus_t   servStatus = { 0 };
// 		ddbsdk_servFileInfo_t fileInfo = { 0 };
// 		int cnt = 0;
...
unsigned int __stdcall Thread_test_getDecrypDataTs(void* param)
{
	PBYTE buffer;
	DWORD size;
	...
	DDB_IOControl(NULL, IOCTL_DDBSDK_DATA_TS_OPEN, NULL, 0, NULL, 0, NULL);
	while(1) {
		...
		if(getCount > 0)
		{
			FILE  * f_data = fopen("\\MMC_Storage\\temp\\DecrypDataTs.ts", "ab");
			//fwrite(tempHeader, 1, 5, f_data);
			fwrite(buffer, 1, getCount, f_data);
			fclose(f_data);
			tmpCount++;
		...
		if((testCount & 0x7) == 0) {
			printf("test_getDecrypDataTs ==> count: %d, bytes: %d, tmpCount: %d\n", testCount, getCount, tmpCount);
			
		}
		if(tmpCount == 100)break;
	}
	DDB_IOControl(NULL, IOCTL_DDBSDK_DATA_TS_CLOSE, NULL, 0, NULL, 0, NULL);
...
void getUUID()
{
	...
	//bRetVal = KernelIoControl (IOCTL_HAL_GET_UUID, NULL, 0, &myUUID,  sizeof (myUUID), &dwBytesReturned);
...
/*	ddbif_videoTest(1);*/
	...
	buffer7 = (PBYTE)malloc(size);
	memset(buffer7, 0, size);

	myBufferStru myBufferInfo1, myBufferInfo2, myBufferInfo3, myBufferInfo4, myBufferInfo5, myBufferInfo6, myBufferInfo7;
	...
	myBufferInfo7.buffer = buffer7;
	myBufferInfo7.size = size;
	...
	unsigned int dwThreadID_0, dwThreadID_1, dwThreadID_2, dwThreadID_3, dwThreadID_4, dwThreadID_5, dwThreadID_6, dwThreadID_7, dwThreadID_8, dwThreadID_9, dwThreadID_10, dwThreadID_11, dwThreadID_12, dwThreadID_13, dwThreadID_14;

	//hThread_0 = _beginthreadex(NULL, 0, thread_sendStreamData, NULL, 0, &dwThreadID_0);

	//hThread_1 = _beginthreadex(NULL, 0, (LPTHREAD_START_ROUTINE)Thread_test_getTrafficData, &myBufferInfo1, 0, &dwThreadID_1);
	//hThread_2 = _beginthreadex(NULL, 0, (LPTHREAD_START_ROUTINE)Thread_test_getPOIinfo, &myBufferInfo2, 0, &dwThreadID_2);
	//hThread_3 = _beginthreadex(NULL, 0, (LPTHREAD_START_ROUTINE)Thread_test_getFinanceData, &myBufferInfo3, 0, &dwThreadID_3);
	//hThread_4 = _beginthreadex(NULL, 0, (LPTHREAD_START_ROUTINE)Thread_test_getWeahter, &myBufferInfo4, 0, &dwThreadID_4);
	hThread_5 = (HANDLE)_beginthreadex(NULL, 0, Thread_test_downloadFile, NULL, 0, &dwThreadID_5);
	//hThread_6 = _beginthreadex(NULL, 0, (LPTHREAD_START_ROUTINE)Thread_test_getSignal, NULL, 0, &dwThreadID_6);
	//hThread_7 = _beginthreadex(NULL, 0, (LPTHREAD_START_ROUTINE)Thread_test_getCarPark, &myBufferInfo5, 0, &dwThreadID_7);
	//hThread_8 = _beginthreadex(NULL, 0, (LPTHREAD_START_ROUTINE)Thread_test_WeatherFile, NULL, 0, &dwThreadID_8);
	//hThread_9 = _beginthreadex(NULL, 0, (LPTHREAD_START_ROUTINE)Thread_test_EmergencyFile, NULL, 0, &dwThreadID_9);
	//hThread_10 = _beginthreadex(NULL, 0, (LPTHREAD_START_ROUTINE)Thread_test_RoadConditionsFile, NULL, 0, &dwThreadID_10);
	//hThread_11 = _beginthreadex(NULL, 0, (LPTHREAD_START_ROUTINE)Thread_test_getTsStream, &myBufferInfo6, 0, &dwThreadID_11);
	//hThread_12 = _beginthreadex(NULL, 0, (LPTHREAD_START_ROUTINE)Thread_test_ExpoFile, NULL, 0, &dwThreadID_12);
	//hThread_13 = _beginthreadex(NULL, 0, (LPTHREAD_START_ROUTINE)Thread_test_TravelFile, NULL, 0, &dwThreadID_13);
	//hThread_14 = (HANDLE)_beginthreadex(NULL, 0, Thread_test_getDecrypDataTs, &myBufferInfo7, 0, &dwThreadID_14);
