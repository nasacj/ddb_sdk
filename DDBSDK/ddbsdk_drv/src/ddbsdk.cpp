...
static HANDLE hCheckSignal = NULL;
...
BOOL suspendAllDownloadingFile()
{
	if (ddbsdk.hDownloadFile != 0 && ((ddbsdk_serviceFile*)ddbsdk.hDownloadFile)->isThreadSuppended != TRUE)
	{
		((ddbsdk_serviceFile*)ddbsdk.hDownloadFile)->isThreadSuppended = TRUE;
	}
	if (ddbsdk.hWeatherFile != 0 && ((ddbsdk_serviceFile*)ddbsdk.hWeatherFile)->isThreadSuppended != TRUE)
	{
		((ddbsdk_serviceFile*)ddbsdk.hWeatherFile)->isThreadSuppended = TRUE;
	}
	if (ddbsdk.hRoadConditionFile != 0 && ((ddbsdk_serviceFile*)ddbsdk.hRoadConditionFile)->isThreadSuppended != TRUE)
	{
		((ddbsdk_serviceFile*)ddbsdk.hRoadConditionFile)->isThreadSuppended = TRUE;
	}
	if (ddbsdk.hEmergencyBroadcastFile != 0 && ((ddbsdk_serviceFile*)ddbsdk.hEmergencyBroadcastFile)->isThreadSuppended != TRUE)
	{
		((ddbsdk_serviceFile*)ddbsdk.hEmergencyBroadcastFile)->isThreadSuppended = TRUE;
	}
	if (ddbsdk.hTravelFile != 0 && ((ddbsdk_serviceFile*)ddbsdk.hTravelFile)->isThreadSuppended != TRUE)
	{
		((ddbsdk_serviceFile*)ddbsdk.hTravelFile)->isThreadSuppended = TRUE;
	}
	if (ddbsdk.hExpoFile != 0 && ((ddbsdk_serviceFile*)ddbsdk.hExpoFile)->isThreadSuppended != TRUE)
	{
		((ddbsdk_serviceFile*)ddbsdk.hExpoFile)->isThreadSuppended = TRUE;
	}
	return TRUE;
}

BOOL resumeAllDownloadingFile()
{
	if (ddbsdk.hDownloadFile != 0 && ((ddbsdk_serviceFile*)ddbsdk.hDownloadFile)->isThreadSuppended == TRUE)
	{
		ResumeThread(((ddbsdk_serviceFile*)ddbsdk.hDownloadFile)->hCheckServiceFile);
		((ddbsdk_serviceFile*)ddbsdk.hDownloadFile)->isThreadSuppended = FALSE;
	}
	if (ddbsdk.hWeatherFile != 0 && ((ddbsdk_serviceFile*)ddbsdk.hWeatherFile)->isThreadSuppended == TRUE)
	{
		ResumeThread(((ddbsdk_serviceFile*)ddbsdk.hWeatherFile)->hCheckServiceFile);
		((ddbsdk_serviceFile*)ddbsdk.hWeatherFile)->isThreadSuppended = FALSE;
	}
	if (ddbsdk.hRoadConditionFile != 0 && ((ddbsdk_serviceFile*)ddbsdk.hRoadConditionFile)->isThreadSuppended == TRUE)
	{
		ResumeThread(((ddbsdk_serviceFile*)ddbsdk.hRoadConditionFile)->hCheckServiceFile);
		((ddbsdk_serviceFile*)ddbsdk.hRoadConditionFile)->isThreadSuppended = FALSE;
	}
	if (ddbsdk.hEmergencyBroadcastFile != 0 && ((ddbsdk_serviceFile*)ddbsdk.hEmergencyBroadcastFile)->isThreadSuppended == TRUE)
	{
		ResumeThread(((ddbsdk_serviceFile*)ddbsdk.hEmergencyBroadcastFile)->hCheckServiceFile);
		((ddbsdk_serviceFile*)ddbsdk.hEmergencyBroadcastFile)->isThreadSuppended = FALSE;
	}
	if (ddbsdk.hTravelFile != 0 && ((ddbsdk_serviceFile*)ddbsdk.hTravelFile)->isThreadSuppended == TRUE)
	{
		ResumeThread(((ddbsdk_serviceFile*)ddbsdk.hTravelFile)->hCheckServiceFile);
		((ddbsdk_serviceFile*)ddbsdk.hTravelFile)->isThreadSuppended = FALSE;
	}
	if (ddbsdk.hExpoFile != 0 && ((ddbsdk_serviceFile*)ddbsdk.hExpoFile)->isThreadSuppended == TRUE)
	{
		DWORD ret = ResumeThread(((ddbsdk_serviceFile*)ddbsdk.hExpoFile)->hCheckServiceFile);
		((ddbsdk_serviceFile*)ddbsdk.hExpoFile)->isThreadSuppended = FALSE;
	}
	return TRUE;
}
...
#if DEBUG_TS_FILE
	fwrite(pBuffer, 1, BufferSize, temp_tsfile);
#endif
#if 0
	...
	*/
	...
	if(ddbsdk.hPAT) {
		...
		while(1) {
			...
			{
				ddbsdk.pat_open = 0;
				//WaitForSingleObject(((ddbsdk_pat *)ddbsdk.hPAT)->hExitEvent, INFINITE);
				Sleep(100);
				delete ddbsdk.hPAT;
				ddbsdk.hPAT = 0;
				return false;
	...
	for(num=0; num<progNum; num++) {
		if(ddbsdk.hPMT) {
			...
			while(1) {
				((ddbsdk_pat *)ddbsdk.hPMT)->getPlainPkgData(tempBuf, 188, &retBytes);
				if(retBytes > 0) {
					break;
				} else {
					timeOutCount ++;
				...
				{
					ddbsdk.pmt_open = 0;
					//WaitForSingleObject(((ddbsdk_pmt *)ddbsdk.hPMT)->hExitEvent, INFINITE);
					Sleep(100);
					delete ddbsdk.hPMT;
					ddbsdk.hPMT = 0;
					return false;
	...
	if(haveCAFlag != 0) { /* if have ECM PID, to check CA for EMM pid */
		...
		if(ddbsdk.hCAT) {
			...
			while(1) {
				...
				{
					ddbsdk.cat_open = 0;
					//WaitForSingleObject(((ddbsdk_cat *)ddbsdk.hCAT)->hExitEvent, INFINITE);
					Sleep(100);
					delete ddbsdk.hCAT;
					ddbsdk.hCAT = 0;
					return false;
...
{
	WriteLog("This is DDB_Init===>\n");
	if(devHandle)
	{
		WriteLog("devHandle!=0 return 0;\n");
		WriteLog("This is DDB_Init===>返回\n");
		return 0;
	}
...
{
//	if(ddbsdk.printDebugOpen) printf("This is DllEntry\n");
    switch ( dwReason ) 
	{
...
{
	WriteLog("This is DDB_Close===>\n");
//	if(ddbsdk.printDebugOpen) printf("This is DAB_Close\n");
...
{
	WriteLog("This is DDB_Deinit===>\n");
//	if(ddbsdk.printDebugOpen) printf("This is DAB_Deinit:");
	...
	WriteLog("This is DDB_Deinit===>返回\n");
    return TRUE;
}
...
{
//	printf("This is DAB_IOControl:%d\n", dwIoControlCode);
	...
	case IOCTL_DDBSDK_OPEN_DMBDEVICE:
		...
		WriteLog("IOCTL_DDBSDK_OPEN_DMBDEVICE==========>\n");
		...
		/* check TDT, to get date */
		{
			...
			ddbsdk.tdt_open = 0;
			//WaitForSingleObject(((ddbsdk_tdt *)ddbsdk.hTDT)->hExitEvent, INFINITE);
			Sleep(100);
			delete ddbsdk.hTDT;
			ddbsdk.hTDT = 0;
...
#ifdef DEBUG
		/* set the expire: 2010.10.01 */
		if(ddbsdk.mjdDate > 0xd8ae) {
			stopDmbService();
			ddbsdk.dataService_open = 0;
			//ddbsdk.device_open = 0;
			WriteLog("DDBSDK过期！\n");
	...
	case IOCTL_DDBSDK_GET_DATETIME:
		{
			...
			if(timeOutCount >= 10) 
			{
				//printf("IOCTL_DDBSDK_GET_DATETIME获取时间超时\n");
				errorMessageOccur(0x2);
				return 0;
			}
	...
	case IOCTL_DDBSDK_TS_STREAM_GET :
		//printf("IOCTL_DDBSDK_TS_STREAM_GET\n");
		if(ddbsdk.ts_Stream) {
	...
	case IOCTL_DDBSDK_CLOSE_DATA_DOWNLOADFILE:
		...
		//WaitForSingleObject(((ddbsdk_serviceFile *)ddbsdk.hServiceFile)->hExitEvent, INFINITE);
		Sleep(100);
	...
	case IOCTL_DDBSDK_CLOSE_WEATHER_DOWNLOADFILE:
		...
		//WaitForSingleObject(((ddbsdk_serviceFile *)ddbsdk.hWeatherFile)->hExitEvent, INFINITE);
		Sleep(100);
	...
	case IOCTL_DDBSDK_CLOSE_ROADCONDITION_DOWNLOADFILE:
		...
		//WaitForSingleObject(((ddbsdk_serviceFile *)ddbsdk.hRoadConditionFile)->hExitEvent, INFINITE);
		Sleep(100);
	...
	case IOCTL_DDBSDK_CLOSE_EMERGENCYBRO_DOWNLOADFILE:
		...
		//WaitForSingleObject(((ddbsdk_serviceFile *)ddbsdk.hEmergencyBroadcastFile)->hExitEvent, INFINITE);
		Sleep(100);
	...
	case IOCTL_DDBSDK_CLOSE_TRAVELFILE_DOWNLOADFILE:
		...
		//WaitForSingleObject(((ddbsdk_serviceFile *)ddbsdk.hTravelFile)->hExitEvent, INFINITE);
		Sleep(100);
	...
	case IOCTL_DDBSDK_CLOSE_EXPOFILE_DOWNLOADFILE:
		...
		//WaitForSingleObject(((ddbsdk_serviceFile *)ddbsdk.hExpoFile)->hExitEvent, INFINITE);
		Sleep(100);
...
/*			ddbsdk_servFileTab_t * g_servTab = &((ddbsdk_serviceFile *)ddbsdk.hServiceFile)->g_servTab;*/
	...
	case IOCTL_DDBSDK_CLOSE_DATA_TRAFFIC :
		ddbsdk.traffic_open = 0;
		//WaitForSingleObject(((ddbsdk_traffic *)ddbsdk.hTraffic)->hExitEvent, INFINITE);
		Sleep(100);
		delete ddbsdk.hTraffic;
		ddbsdk.hTraffic = 0;
	...
	case IOCTL_DDBSDK_CLOSE_DATA_CARPARK:
		...
		//WaitForSingleObject(((ddbsdk_carPark *)ddbsdk.hCarPark)->hExitEvent, INFINITE);
		Sleep(100);
	...
	case IOCTL_DDBSDK_SUSPEND_ALL_DOWNLOADFILE:
		//		suspendAllDownloadingFile();
		if (suspendAllDownloadingFile())
		{
			printf("downloading file suspended!\n");
		}
		else
		{
			printf("downloading file suspend failed!\n");
		}
		break;
	case IOCTL_DDBSDK_RESUM_ALL_DOWMLOADFILE:
		//		resumeAllDownloadingFile();
		if (resumeAllDownloadingFile())
		{
			printf("downloading file resumed!\n");
		}
		else
		{
			printf("downloading file resume failed!\n");
		}
		break;

	default:
//		if(ddbsdk.printDebugOpen) printf("donot surport IOCTL!\n");
		return 0;
	}
//printf("total: %d\n", bytesRet);
    return TRUE;
...
{
//	if(ddbsdk.printDebugOpen) printf("This is DAB_Read\n");

	return DWORD(1);;
}
DWORD WINAPI DDB_Write(DWORD Handle, LPCVOID pBuffer, DWORD dwNumBytes)
{
//	if(ddbsdk.printDebugOpen) printf("This is DAB_Write\n");
	return DWORD(1);;
}
DWORD WINAPI DDB_Seek(DWORD Handle, long lDistance, DWORD dwMoveMethod)
{
//	if(ddbsdk.printDebugOpen) printf("This is DAB_Seek\n");
	return DWORD(1);;
}
...
// 		printf("This is DAB_PowerDown\n");
	RETAILMSG(1, (TEXT("DDB_PowerDown.........\r\n"))); 
	return;
}
...
int InitMessageMapMutex()
{
	...
	if(messageEvent == NULL) 
		//printf("messageEvent == NULL!\n");
	if(messageMutex == NULL) 
		//printf("messageMutex == NULL!\n");
	if(messageFileMap == NULL) 
		//printf("messageFileMap == NULL!\n");
	if(messageEvent == NULL || messageMutex == NULL || messageFileMap == NULL) 
		return -1;
