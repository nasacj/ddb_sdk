...
uint32_t pushPacketVideoPES2(demux_if_t * param1, uint8_t * buf)
{
	...
	if (pes_pack.f_pes_first_pack == 1) {  //for PES start PACK
		...
		printf("pes_packet_length = %x = %d (bytes)\n",pes_pack.pes_packet_length,pes_pack.pes_packet_length);

		printf("stream_id:");
		switch (stream_id) {
		case PROGRAM_STREAM_MAP:printf("PROGRAM_STREAM_MAP:%x\n",PROGRAM_STREAM_MAP);
		case PRIVATE_STREAM_2:printf("PRIVATE_STREAM_2:%x\n",PRIVATE_STREAM_2);
		case ECM_STREAM:printf("ECM_STREAM:%x\n",ECM_STREAM);
		case EMM_STREAM:printf("EMM_STREAM:%x\n",EMM_STREAM);
		case PROGRAM_STREAM_DIRECTORY:printf("PROGRAM_STREAM_DIRECTORY:\%xn",PROGRAM_STREAM_DIRECTORY);
			break;
		case PADDING_STREAM:printf("PADDING_STREAM:%x\n",PADDING_STREAM);
		...
		default:
			if((stream_id >= 0xF4 && stream_id <= 0xFE) || stream_id < 0xBC) {
				...
				if(stream_id == SEC14496_SL_PACKED_STREAM) {
					...
					printf("SEC14496_SL_PACKED_STREAM:%x\n",SEC14496_SL_PACKED_STREAM);
					...
					printf("输出字节数：%x = %d (bytes)\n",pack_copy_len,pack_copy_len);
