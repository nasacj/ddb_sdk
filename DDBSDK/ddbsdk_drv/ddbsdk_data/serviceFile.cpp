...
//						printf("file open error!\n");
					...
					pServFileDowning->statFilePos = statFilePos;

					memcpy(&stat_format, &synt, sizeof(ddbsdk_servFile_syntax_t));
					stat_format.block_num_down = 0;
					stat_format.down_stat_bytes = (synt.Block_num + 7) / 8;
...
DONOT_CREATE_TAB:
				...
				if(statPtr->numDown * synt.Block_length >= BLOCKDOWNBUFSIZE || statPtr->numDown >= leftNum) { 
					int   blk_num_pos, stat_byte_pos, stat_bit_pos;
					/* check if file exist */
				...
				//printf("service file download crc error!\n");
			}
		}
	}
...
ddbsdk_serviceFile::initServiceFileData(void)
{
	...
	((ddbsdk_serviceFile *)ddbsdk.hServiceFile)->ecm_pid = 4118;//4117;
