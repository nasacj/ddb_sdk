...
//				if(ddbsdk.printDebugOpen) printf("TimeOut\n");
				Sleep(300);
				* pBytesReturned = 0;
				return 0;
			}
...
//				if(ddbsdk.printDebugOpen) printf("Tx Sync error!\n");
				Sleep(300);
				* pBytesReturned = 0;
				return 0;
			}
...
//				if(ddbsdk.printDebugOpen) printf("Tx Sync error!\n");
			...
			if(bytesRet + length > nOutBufSize) {
				Sleep(1000);
//				if(ddbsdk.printDebugOpen) printf("too long pkg, overflowed!\n");
				* pBytesReturned = 0;
				return 0;
			}
...
//					if(ddbsdk.printDebugOpen) printf("TimeOut\n");
			...
			cc = plainTsPtrOut[3] & 0xf;
			if(cc != ((prev_cc + 1) & 0xF)) {
//				if(ddbsdk.printDebugOpen) printf("continue counter error!\n");
				Sleep(300);
				* pBytesReturned = 0;
				return 0;
			}
...
//			if(ddbsdk.printDebugOpen) printf("error, pkgLen: %d, getBytes: %d\n", pkgLength, bytesRet);
			* pBytesReturned = 0;
		} else {
