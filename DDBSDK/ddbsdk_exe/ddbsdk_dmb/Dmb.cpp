...
long DmbStart(int   frequency)
{
	...
	WriteLog("Got:%d components",nReturnedComponents);
	...
	U8ToUnicode((char*)components_f[0].ServiceInfo.pLabel);
	U8ToUnicode((char*)components_f[1].ServiceInfo.pLabel);
	U8ToUnicode((char*)components_f[2].ServiceInfo.pLabel);
	U8ToUnicode((char*)components_f[3].ServiceInfo.pLabel);
	U8ToUnicode((char*)components_f[4].ServiceInfo.pLabel);
	...
	printf("Request No.%d Stream~~~~~~", index);
...
#endif
	WriteLog("DMB Start Successfully!");
	printf("DMB Start Successfully!\n");
	return 0;
}
...
//		Wait_CallBack(hEventEnsembleChanged,1000);
		...
		UINT32 ServiceID = components[index2].ServiceInfo.SID;//0xf0c00030;
